...
function DegisimBadge({ pct }: { pct: number | null }) {
  ...
  const renk = pct > 0 ? "text-green-600" : pct < 0 ? "text-orange-500" : "text-slate-400";
  return (
  ...
  return (
    <div className="w-full">
      ...
      <input
        ...
        className="mb-6 w-full rounded-lg border border-slate-300 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
      />
      ...
      <div className="space-y-2">
        {filtreliGruplar.map((g) => {
          ...
          return (
          <div key={g.model} className="rounded-xl border border-slate-200 bg-white overflow-hidden">
            {/* Grup başlığı */}
            <button
            ...
            {acik && <div className="divide-y divide-slate-100 border-t border-gray-100">
              {g.tipler.map((tip) => (
                <div key={tip.tip_kodu} className="flex items-start justify-between gap-4 px-4 py-3">
                  {/* Sol: araç adı + spek chip'leri + CTA */}
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-slate-900 leading-snug">{tip.tipAdi}</p>
                    {tip.spek && (
                      <div className="mt-1 flex flex-wrap gap-1">
                        {tip.spek.yakit && <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-600">{tip.spek.yakit}</span>}
                        {tip.spek.motor_guc_hp && <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-600">{tip.spek.motor_guc_hp} hp</span>}
                        {tip.spek.motor_hacmi && <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-600">{tip.spek.motor_hacmi}L</span>}
                        {tip.spek.vites && <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-600">{tip.spek.vites}</span>}
                        {tip.spek.kasa && <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-600">{tip.spek.kasa}</span>}
                        {tip.spek.cekis && tip.spek.cekis !== "2wd" && <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-600">{tip.spek.cekis}</span>}
                      </div>
                    )}
                    <button
                  ...
                  <div className="shrink-0 text-left">
                    <p className="text-base font-bold text-slate-900 tabular-nums">{formatTL(tip.deger)}</p>
                    {tip.oncekiAy && (
